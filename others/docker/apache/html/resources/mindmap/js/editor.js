...
function loadDesignerOptions(jsonConf) {
    ...
    if (jsonConf) {
        ...
        var isLocalhost = (window.location.host.indexOf('localhost') !== -1 || window.location.host.indexOf('127.0.0.1') !== -1)

        // Parse parameters
        var params = new URL(window.location.href).searchParams;
        var readOnly = isLocalhost ? false : ('true' == params.get('readonly'));
        ...
        if (!isLocalhost) {
            // Instantiate Nero persistence manager
            var backendUrl = 'https://' + window.location.host + '/lfr/api/jsonws/document.mindmap';
            var neroPersistenceManager = new mindplot.NeroPersistenceManager(backendUrl);

            var fileVersionId = params.get('fileVersionId');

            result.persistenceManager = neroPersistenceManager,
            result.mapId = fileVersionId
        }
    }
    return result;
}
